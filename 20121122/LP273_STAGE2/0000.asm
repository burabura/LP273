...
BOOL Fn_ADsampleOneShot(BOOL* flag)
{
	...
	#ifdef F9224EV
	AD_SelectADChannel(AD_INPUT_CHANNEL0);
	#endif
...
/*HARDWERA initial*/	
	...
	{	
		...
		while(flagP32LOW)	//(b)
		{
				...
				if((StopMode5sCount==0x1262)||(P3.2==1)) //4706x255x4us = 5s ,4706=0x1262
				{
					StopMode5sCount=0;
					flagP32LOW=0; 	   // (b)
					flag8hours=1;
...
/*FirstOpenTest 30s light*/
		...
		else if(flagFirstOpenTest)	//(c)
		{
			...
			{
				if(DuringINC_count<1)		//feng pin
					DuringINC_count++;
				else
				{
					gDuty+=1;
					DuringINC_count=0;
					TMH1_ChangeDuty(gDuty);
				}	
			}
			goto OVER_BEING;
		}
